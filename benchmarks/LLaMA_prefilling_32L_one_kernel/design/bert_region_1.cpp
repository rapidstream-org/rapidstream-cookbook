    ...
    #pragma HLS DATAFLOW
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          ap_int<32> outp0_dp = outp_temp.range(31, 0);
          ap_int<32> outp1_dp = outp_temp.range(63, 32);
          ap_int<8> outp0 = outp0_dp * buf19[ps_offset + i];
          ap_int<8> outp1 = outp1_dp * buf19[ps_offset + i];
          outp_data_pack_0.range(i*8 + 7, i*8) = outp0;
          outp_data_pack_1.range(i*8 + 7, i*8) = outp1;
        }
        double_pkt_int8 pkt_temp;
    ...
    #pragma HLS DATAFLOW
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          ap_int<32> outp0_dp = outp_temp.range(31, 0);
          ap_int<32> outp1_dp = outp_temp.range(63, 32);
          ap_int<8> outp0 = outp0_dp * buf20[ps_offset + i];
          ap_int<8> outp1 = outp1_dp * buf20[ps_offset + i];
          outp_data_pack_0.range(i*8 + 7, i*8) = outp0;
          outp_data_pack_1.range(i*8 + 7, i*8) = outp1;
        }
        double_pkt_int8 pkt_temp;
  ...
  l_pack_seq: for (int ps_id = 0; ps_id < pack_seq_num_inp; ps_id++){
    ...
    #pragma HLS pipeline II=1
      ...
      l_load_i: for (int i = 0; i < inp_parallel; i++) {
        ...
        data_temp.f *= buf17[ps_id * inp_parallel + i];
...
void Bert_layer_dataflow_region_1(
  ...
  input_loader_kv(inp_addr_1, inp_k, inp_v);  // L457
  input_loader_q(inp_addr_2, inp_q);  // L457
  weight_loader_r1(wk_addr, wv_addr, wq_addr, block_wk_loader, block_wv_loader, block_wq_loader);
  Linear_layer_qkv1(inp_k, block_wk_loader, outp_k);  // L458
  Linear_layer_qkv2(inp_v, block_wv_loader, outp_v);  // L459
  Linear_layer_qkv3(inp_q, block_wq_loader, outp_q);  // L460
}
