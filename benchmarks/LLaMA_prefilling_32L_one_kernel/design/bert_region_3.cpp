    ...
    for(int jj = 0; jj < pack_gelu_len_w / 2; jj++){
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          converter_t outp0;
          converter_t outp1;
          outp0.f = outp0_dp * buf26[ps_offset + i];
          outp1.f = outp1_dp * buf26[ps_offset + i];
...
void Linear_layer_ds1(
  ...
  hls::stream<double_io_pack_float>& outp
    ...
    #pragma HLS dataflow
    Linear_layer_ds1_sub1(inp, block_A_loader);
    Linear_layer_ds1_sub2(block_A_loader, block_B_loader, block_C_drainer);
    Linear_layer_ds1_sub3(block_C_drainer, outp);

}



void Linear_layer_ds2_sub1(
  hls::stream<io_pack_int8>& inp,
  hls::stream<io_pack_int8>& block_A_loader
){
    ...
    for(int jj = 0; jj < pack_inp_len_w / 2; jj++){
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          converter_t outp0;
          converter_t outp1;
          outp0.f = outp0_dp * buf28[ps_offset + i];
          outp1.f = outp1_dp * buf28[ps_offset + i];
          outp_data_pack_0.range(i*32 + 31, i*32) = outp0.i;
          outp_data_pack_1.range(i*32 + 31, i*32) = outp1.i;
        }
        outp_data_pack = (outp_data_pack_1, outp_data_pack_0);
...
void Linear_layer_ds2(
  ...
  hls::stream<double_io_pack_float>& outp
    ...
    #pragma HLS dataflow
    Linear_layer_ds2_sub1(inp, block_A_loader);
    Linear_layer_ds2_sub2(block_A_loader, block_B_loader, block_C_drainer);
    Linear_layer_ds2_sub3(block_C_drainer, outp);
    ...
    l_pack_seq: for (int ps_id = 0; ps_id < pack_seq_num_inp; ps_id++){
        ...
        #pragma HLS pipeline II=1
            ...
            l_loader_i: for (int i = 0; i < inp_parallel; i++) {        // L4
                ...
                data_temp.f *=  buf25[ps_id * inp_parallel + i];   // L9
    ...
    #pragma HLS DATAFLOW
        ...
        #pragma HLS pipeline II=1
            ...
            l_i20: for (int i20 = 0; i20 < inp_parallel; i20++) {   // L322
                ...
                outp_data_pack.range(i20*8 + 7, i20*8) = ap_int<8>(outp_data * buf27[ps_offset + i20]);
            }
            outp.write(outp_data_pack);
        }
    }
...
#include "const/buf16.h"
    ...
    l_pack_seq: for (int ps_id = 0; ps_id < pack_seq_num_inp; ps_id++){
        ...
        #pragma HLS pipeline II=1
            ...
            data_temp.f = data_temp.f * buf15[j16] + buf16[j16];
...
void Bert_layer_dataflow_region_3(
    ...
    #pragma HLS STREAM variable=outp_gelu depth=11008
