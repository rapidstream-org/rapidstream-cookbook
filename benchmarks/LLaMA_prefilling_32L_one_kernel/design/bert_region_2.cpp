...
void systolic_array_attn(
  ...
  hls::stream<ap_int<64>> C[block_size_a][block_size_b];
	...
	data_load_B:for (int k = 0; k < head_len; k++) {
	#pragma HLS PIPELINE II=1
	...
	data_drain_A:for (int k = 0; k < head_len; k++) {
	#pragma HLS PIPELINE II=1
		for (int m = 0; m < block_size_a; m++) {
	...
	data_drain_B:for (int k = 0; k < head_len; k++) {
	#pragma HLS PIPELINE II=1
		for (int n = 0; n < block_size_b; n++) {
...
void systolic_array_cont(
  ...
  hls::stream<ap_int<64>> C[block_size_a][block_size_b];
	...
	data_load_B:for (int k = 0; k < seq_num; k++) {
	#pragma HLS PIPELINE II=1
        io_pack_int8 B_temp = B_loader.read();
		for (int n = 0; n < block_size_b; n++) {
			B_fifo[n][0].write(B_temp.range(n*16 + 15, n*16));
	...
	data_drain_A:for (int k = 0; k < seq_num; k++) {
	#pragma HLS PIPELINE II=1
		for (int m = 0; m < block_size_a; m++) {
			A_fifo[m][block_size_b].read();
		}
	...
	data_drain_B:for (int k = 0; k < seq_num; k++) {
	#pragma HLS PIPELINE II=1
		for (int n = 0; n < block_size_b; n++) {
			B_fifo[n][block_size_a].read();
		}
...
#pragma HLS array_partition variable=buf21 cyclic factor=8
    ...
    Attention_layer_sub1(inp, block_A_loader);
    Attention_layer_sub2(block_A_loader, block_B_loader, block_C_drainer);
    Attention_layer_sub3(block_C_drainer, outp);
            ...
            for(int jj = 0; jj < pack_head_len_w; jj++){
                ...
                #pragma HLS pipeline II=1
                    ...
                    l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
                        ...
                        ap_int<32> outp0_dp = outp_temp.range(31, 0);
                        ap_int<32> outp1_dp = outp_temp.range(63, 32);
                        ap_int<8> outp0 = outp0_dp * buf23[ps_offset + i];
                        ap_int<8> outp1 = outp1_dp * buf23[ps_offset + i];
    ...
    for(int jj = 0; jj < pack_inp_len_w; jj++){
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          converter_t outp0;
          converter_t outp1;
          outp0.f = outp0_dp * buf24[ps_offset + i];
          outp1.f = outp1_dp * buf24[ps_offset + i];
          outp_data_pack_0.range(i*32 + 31, i*32) = outp0.i;
          outp_data_pack_1.range(i*32 + 31, i*32) = outp1.i;
        }
        outp_data_pack = (outp_data_pack_1, outp_data_pack_0);
...
void Linear_layer_ds0(
  ...
  hls::stream<double_io_pack_float>& outp
    ...
    Linear_layer_ds0_sub1(inp, block_A_loader);
    Linear_layer_ds0_sub2(block_A_loader, block_B_loader, block_C_drainer);
    Linear_layer_ds0_sub3(block_C_drainer, outp);
        ...
        l_multi_head: for (int h = 0; h < head_num; h++){
            ...
            #pragma HLS pipeline II=1
                l_update_i7: for (int i7 = 0; i7 < inp_parallel; i7++) {        // L102
                    ...
                    int8_t v80 = v78 * buf22[ps_id * inp_parallel + i7];    // L108
...
#include "const/buf14.h"
    ...
    l_pack_seq: for (int ps_id = 0; ps_id < pack_seq_num_inp; ps_id++){
        ...
        #pragma HLS pipeline II=1
            ...
            data_temp.f = data_temp.f * buf13[j16] + buf14[j16];
...
void Bert_layer_dataflow_region_2(
    ...
    K_writer_Attention_layer(outp_k, inp_sfa, attn_outp);
    Softmax_layer(attn_outp, sfm_outp);
    V_writer_Context_layer(outp_v, sfm_outp, outp_sfa);
    Linear_layer_ds0(outp_sfa, block_w_ds0_loader, outp_ds0);
    Res_layer0(outp_ds0, outp_inp, outp_res0);
    Layer_norm0(outp_res0, outp_ln0);
}
