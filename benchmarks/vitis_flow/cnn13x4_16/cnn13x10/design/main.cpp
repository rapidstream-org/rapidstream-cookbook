      ...
      for (int c4 = 0; c4 <= 12; c4 += 1) {
        ...
        for (int c5 = 0; c5 <= 15; c5 += 1)
          for (int c6 = 0; c6 <= 63; c6 += 1)
            C_to[(16 * c4 + c5) * 640 + (64 * c3 + c6)] = C_from[cnt++];
      }
    }
}
/* Helper Function */
...
//  data_t A[I][K], B[K][J], C[I][J], C_golden[I][J];
...

  {
    ...
    float *dev_B_unserialized = (float *)malloc((640) * (256) * sizeof(float));
    float *dev_B = (float *)malloc(163840 * sizeof(float));
    float *dev_C_unserialized = (float *)malloc((208) * (640) * sizeof(float));
    float *dev_C = (float *)malloc(133120 * sizeof(float));

    // Initialize host buffers
    memcpy(dev_A_unserialized, A, (208) * (256) * sizeof(float));
    memcpy(dev_B_unserialized, B, (640) * (256) * sizeof(float));
    memcpy(dev_C_unserialized, C, (208) * (640) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
        B_t16 *buffer_B_tmp = (B_t16 *)malloc((163840) * sizeof(float));
        buffer_B.push_back(buffer_B_tmp);
    }
    for (int i = 0; i < 1; i++) {
        C_t16 *buffer_C_tmp = (C_t16 *)malloc((133120) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
        memcpy(buffer_B[i], dev_B, (163840) * sizeof(float));
    }
    ...
    for (int i = 0; i < 1; i++) {
        memcpy(dev_C, buffer_C[i], (133120) * sizeof(float));
    }

    host_deserialize_C(dev_C_unserialized, dev_C);
    // Restore data from host buffers
    memcpy(C, dev_C_unserialized, (208) * (640) * sizeof(float));
