...
void loadBV(
    ...
    for (int i_req = 0, i_resp = 0;
            i_resp < ( (BV_LENGTH-1)/(BV_PACKED_BITWIDTH * (7)) + 1); )
    {
#pragma HLS PIPELINE II=1
        if (i_req < ( (BV_LENGTH-1)/(BV_PACKED_BITWIDTH * (7)) + 1) && input_bv.read_addr.try_write(i_req)) {
            ++i_req;
        }
        if (!input_bv.read_data.empty()) {
            ...
            bv_load_stream_5.write(cur_bv_val.section5);
            bv_load_stream_6.write(cur_bv_val.section6);
            ++i_resp;
        }
    }
    return;
...
void computeHash_Feeder(
    ...
    const int WRITE_STOP_COUNT = KEYPAIRS_PER_STM*(7);
...
#pragma HLS PIPELINE II=1
        bool do_read = 1;
        HASH_RD_LOOP:
        for(int hash_idx = 0; hash_idx < (7); ++hash_idx){
...
void computeHash_Computer(
    ...
    int module_idx = stm_idx*(7) + hash_idx;
...
void bloom_arb_forwarder(
    ...
    const int READ_STOP_COUNT = (4) * KEYPAIRS_PER_STM;
    const int WRITE_STOP_COUNT = KEYPAIRS_PER_STM;
    int total_num_reads = 0;
    int total_num_writes = 0;
    int num_writes_per_stm[(4)];
#pragma HLS ARRAY_PARTITION variable=num_writes_per_stm dim=0 complete
    ...
    */
    static
    ap_uint<INPUT_IDX_BITWIDTH> reads_per_input[(4)];
...
#pragma HLS PIPELINE II=1
        RD_LOGIC:
        for (int strm_idx = 0; strm_idx < (4); ++strm_idx) {
#pragma HLS UNROLL
            // Metadata:
            ...
            else if (!hash_stream[strm_idx].empty())
            {
                ...
                ap_uint<HASHONLY_BITWIDTH> idx_inside_partition = tmp_hash % ( (BV_LENGTH-1)/((7)*(8) /* each sub bv is further partitioned into this chunks*/) + 1);
                int partition_idx = (tmp_hash / ( (BV_LENGTH-1)/((7)*(8) /* each sub bv is further partitioned into this chunks*/) + 1));
                total_num_reads++;
...
#pragma HLS UNROLL
            ...
            {
#pragma HLS UNROLL
                int out_fifo_idx = partition_idx*(4) + strm_idx;
                if (xbar[strm_idx].valid == 1 &&
                    xbar[strm_idx].target_partition_idx == partition_idx)
                {
...
#pragma HLS PIPELINE II=1
    ...
    // With only one variable this takes one more cycle.
        ...
        WR_LOGIC:
        int valid_idxes = 0;
        int allowed_idx_s0 = min_output_idx_s0 + ((16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/);
        int allowed_idx_s1 = min_output_idx_s1 + ((16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/);
        int allowed_idx_s2 = min_output_idx_s2 + ((16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/);
        int allowed_idx_s3 = min_output_idx_s3 + ((16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/);
        if (xbar[0].valid &&
...
void bloom_arbiter_ratemonitor(
     ...
     */
    WRITE_STOP_COUNT = (4) * KEYPAIRS_PER_STM;
...
#pragma HLS ARRAY_PARTITION variable=xbar dim=0 complete
    ap_uint<INPUT_IDX_BITWIDTH> min_output_idx[(4)];
#pragma HLS ARRAY_PARTITION variable=min_output_idx dim=0 complete
    ap_uint<BIT_BITWIDTH> idx_tracker[(4)][(16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/];
#pragma HLS ARRAY_PARTITION variable=idx_tracker dim=0 complete
    ...
    for (int i = 0; i < (4); ++i) {
        min_output_idx[i] = 0;
        for (int j = 0; j < (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; ++j) {
            idx_tracker[i][j] = 0;
        }
    }
    MAIN_LOOP:
...
#pragma HLS PIPELINE II=1
        ...
        // WR_OUTPUTS:
        ///////////////////////
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[0].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[0].valid && xbar[0].value.md.sidx == sidx && !arb_stream_out[0].full()) { xbar[0].valid = 0; arb_stream_out[0].write(xbar[0].value); idx_tracker[sidx][offset] = 1; writes_per_partition[0]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[1].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[1].valid && xbar[1].value.md.sidx == sidx && !arb_stream_out[1].full()) { xbar[1].valid = 0; arb_stream_out[1].write(xbar[1].value); idx_tracker[sidx][offset] = 1; writes_per_partition[1]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[2].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[2].valid && xbar[2].value.md.sidx == sidx && !arb_stream_out[2].full()) { xbar[2].valid = 0; arb_stream_out[2].write(xbar[2].value); idx_tracker[sidx][offset] = 1; writes_per_partition[2]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[3].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[3].valid && xbar[3].value.md.sidx == sidx && !arb_stream_out[3].full()) { xbar[3].valid = 0; arb_stream_out[3].write(xbar[3].value); idx_tracker[sidx][offset] = 1; writes_per_partition[3]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[4].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[4].valid && xbar[4].value.md.sidx == sidx && !arb_stream_out[4].full()) { xbar[4].valid = 0; arb_stream_out[4].write(xbar[4].value); idx_tracker[sidx][offset] = 1; writes_per_partition[4]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[5].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[5].valid && xbar[5].value.md.sidx == sidx && !arb_stream_out[5].full()) { xbar[5].valid = 0; arb_stream_out[5].write(xbar[5].value); idx_tracker[sidx][offset] = 1; writes_per_partition[5]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[6].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[6].valid && xbar[6].value.md.sidx == sidx && !arb_stream_out[6].full()) { xbar[6].valid = 0; arb_stream_out[6].write(xbar[6].value); idx_tracker[sidx][offset] = 1; writes_per_partition[6]++; break; } }
        for (int sidx = 0; sidx < (4); ++sidx) { int offset = (xbar[7].value.md.iidx) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (xbar[7].valid && xbar[7].value.md.sidx == sidx && !arb_stream_out[7].full()) { xbar[7].valid = 0; arb_stream_out[7].write(xbar[7].value); idx_tracker[sidx][offset] = 1; writes_per_partition[7]++; break; } }
        ///////////////////////
        // UPDATE_IDCES:
        ///////////////////////
        int shuf_idx0 = (min_output_idx[0] + 1) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (idx_tracker[0][shuf_idx0] == 1) { min_output_idx[0] += 1; idx_tracker[0][shuf_idx0] = 0; } /*#ifdef __DO_DEBUG_PRINTS__                    //printf("ARBITER RATEMON %d %c kp%d - Updating min_output_idx[%d]=%d\n",                   //        arb_idx,                  //        ratemon_ID,                   //        kp_idx,                   //        STM,                  //        min_output_idx[STM].to_int()                  //);                    //#endif
*/ int shuf_idx1 = (min_output_idx[1] + 1) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (idx_tracker[1][shuf_idx1] == 1) { min_output_idx[1] += 1; idx_tracker[1][shuf_idx1] = 0; } /*#ifdef __DO_DEBUG_PRINTS__                    //printf("ARBITER RATEMON %d %c kp%d - Updating min_output_idx[%d]=%d\n",                   //        arb_idx,                  //        ratemon_ID,                   //        kp_idx,                   //        STM,                  //        min_output_idx[STM].to_int()                  //);                    //#endif
*/ int shuf_idx2 = (min_output_idx[2] + 1) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (idx_tracker[2][shuf_idx2] == 1) { min_output_idx[2] += 1; idx_tracker[2][shuf_idx2] = 0; } /*#ifdef __DO_DEBUG_PRINTS__                    //printf("ARBITER RATEMON %d %c kp%d - Updating min_output_idx[%d]=%d\n",                   //        arb_idx,                  //        ratemon_ID,                   //        kp_idx,                   //        STM,                  //        min_output_idx[STM].to_int()                  //);                    //#endif
*/ int shuf_idx3 = (min_output_idx[3] + 1) % (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; if (idx_tracker[3][shuf_idx3] == 1) { min_output_idx[3] += 1; idx_tracker[3][shuf_idx3] = 0; } /*#ifdef __DO_DEBUG_PRINTS__                    //printf("ARBITER RATEMON %d %c kp%d - Updating min_output_idx[%d]=%d\n",                   //        arb_idx,                  //        ratemon_ID,                   //        kp_idx,                   //        STM,                  //        min_output_idx[STM].to_int()                  //);                    //#endif
*/
        WRITE_FEEDBACK:
...
void bloom_single_arbiter(
    ...
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p0;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p1;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p2;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p3;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p4;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p5;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p6;
    tapa::streams<RATEMON_FEEDBACK_DTYPE, ((4) - 1)> ratemon_fdbk_streams_p7;
    tapa::task()
        .invoke<tapa::detach>(
                ...
                ,0
                ,kp_idx
                ,in_arb_streams[(4)*0 + 0]
                ,in_arb_streams[(4)*0 + 1]
                ,in_arb_streams[(4)*0 + 2]
                ,in_arb_streams[(4)*0 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,1
                ,kp_idx
                ,in_arb_streams[(4)*1 + 0]
                ,in_arb_streams[(4)*1 + 1]
                ,in_arb_streams[(4)*1 + 2]
                ,in_arb_streams[(4)*1 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,2
                ,kp_idx
                ,in_arb_streams[(4)*2 + 0]
                ,in_arb_streams[(4)*2 + 1]
                ,in_arb_streams[(4)*2 + 2]
                ,in_arb_streams[(4)*2 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,3
                ,kp_idx
                ,in_arb_streams[(4)*3 + 0]
                ,in_arb_streams[(4)*3 + 1]
                ,in_arb_streams[(4)*3 + 2]
                ,in_arb_streams[(4)*3 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,4
                ,kp_idx
                ,in_arb_streams[(4)*4 + 0]
                ,in_arb_streams[(4)*4 + 1]
                ,in_arb_streams[(4)*4 + 2]
                ,in_arb_streams[(4)*4 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,5
                ,kp_idx
                ,in_arb_streams[(4)*5 + 0]
                ,in_arb_streams[(4)*5 + 1]
                ,in_arb_streams[(4)*5 + 2]
                ,in_arb_streams[(4)*5 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,6
                ,kp_idx
                ,in_arb_streams[(4)*6 + 0]
                ,in_arb_streams[(4)*6 + 1]
                ,in_arb_streams[(4)*6 + 2]
                ,in_arb_streams[(4)*6 + 3]
        ...
        .invoke<tapa::detach>(
                ...
                ,7
                ,kp_idx
                ,in_arb_streams[(4)*7 + 0]
                ,in_arb_streams[(4)*7 + 1]
                ,in_arb_streams[(4)*7 + 2]
                ,in_arb_streams[(4)*7 + 3]
...
void queryResult_per_hash(
    ...
    const int MAX_NUM_WRITES = (4)*KEYS_PER_STM;
    int num_writes = 0;
    int num_reads = 0;
    ap_uint<BV_PACKED_BITWIDTH> bv_buf_BRAMS[( ((8) /* each sub bv is further partitioned into this chunks*/-1)/2 + 1)][( (BV_LENGTH-1)/((7)*BV_PACKED_BITWIDTH*(8) /* each sub bv is further partitioned into this chunks*/) + 1)];
...
#pragma HLS ARRAY_PARTITION variable=bv_buf_BRAMS dim=1 complete
    ap_uint<BV_PACKED_BITWIDTH> bv_buf_URAMS[((8) /* each sub bv is further partitioned into this chunks*/ / 2)][( (BV_LENGTH-1)/((7)*BV_PACKED_BITWIDTH*(8) /* each sub bv is further partitioned into this chunks*/) + 1)];
...
#pragma HLS UNROLL
    ...
    LOAD_BV_VALUES:
    for (int i = 0; i < ( (BV_LENGTH-1)/((7)*BV_PACKED_BITWIDTH) + 1); ++i) {
#pragma HLS PIPELINE II=1
        cur_bv_val = bv_load_stream.read();
        int partition_idx = i/( (BV_LENGTH-1)/((7)*BV_PACKED_BITWIDTH*(8) /* each sub bv is further partitioned into this chunks*/) + 1);
        int element_idx = i%( (BV_LENGTH-1)/((7)*BV_PACKED_BITWIDTH*(8) /* each sub bv is further partitioned into this chunks*/) + 1);
...
void shuffle_TtoS_per_hash(
    ...
    PEEKED_DTYPE shuffle_peek_emulator[(8) /* each sub bv is further partitioned into this chunks*/][(4)];
...
)
{
    ...
    PEEKED_DTYPE shufbuf[(8) /* each sub bv is further partitioned into this chunks*/][(16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/];
#pragma HLS ARRAY_PARTITION variable=shufbuf dim=0 complete
    ...
    for (int p = 0; p < (8) /* each sub bv is further partitioned into this chunks*/; ++p) {
        for (int i = 0; i < (16) /* Number of buffered elements we can have for each hash/partition, in the shuffle buffer.*/; ++i) {
            shufbuf[p][i].bv = 0;
            shufbuf[p][i].iidx = 0;
            shufbuf[p][i].valid = 0;
        }
...
)
{
    ...
    tapa::streams<ap_uint<(32)>, (4), (( ((4)-1)/2 + 1))> key_stream_kp0;
    tapa::streams<ap_uint<(32)>, (4), (( ((4)-1)/2 + 1))> key_stream_kp1;
    // loadBV outputs
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_0;
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_1;
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_2;
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_3;
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_4;
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_5;
    tapa::stream<ap_uint<BV_PACKED_BITWIDTH>, (( ((4)-1)/2 + 1))> bv_load_stream_6;
    // Computehash outputs (kp stands for key-pair)
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h0_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h0_kp1;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h1_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h1_kp1;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h2_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h2_kp1;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h3_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h3_kp1;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h4_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h4_kp1;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h5_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h5_kp1;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h6_kp0;
    tapa::streams<ap_uint<HASHONLY_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> hash_stream_h6_kp1;
    // Arbiter outputs
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h0_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h0_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h1_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h1_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h2_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h2_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h3_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h3_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h4_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h4_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h5_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h5_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h6_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> bv_lookup_stream_h6_kp1;
    // Query unit outputs
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash0_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash0_kp1;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash1_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash1_kp1;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash2_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash2_kp1;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash3_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash3_kp1;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash4_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash4_kp1;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash5_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash5_kp1;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash6_kp0;
    tapa::streams<BV_PLUS_METADATA_PACKED_DTYPE, (8) /* each sub bv is further partitioned into this chunks*/, (( ((4)-1)/2 + 1))> query_bv_packed_stream_hash6_kp1;
    // Shuffle unit output
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm0_kp0;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm0_kp1;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm1_kp0;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm1_kp1;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm2_kp0;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm2_kp1;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm3_kp0;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (7), (( ((4)-1)/2 + 1))> reconstruct_stream_stm3_kp1;
    // Aggregate output
    tapa::streams<ap_uint<BIT_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> aggregate_stream_kp0;
    tapa::streams<ap_uint<BIT_BITWIDTH>, (4), (( ((4)-1)/2 + 1))> aggregate_stream_kp1;
    // Datapacked outputs
    tapa::streams<ap_uint<((32))>, (4), (( ((4)-1)/2 + 1))> packed_output_stm_kp0;
    tapa::streams<ap_uint<((32))>, (4), (( ((4)-1)/2 + 1))> packed_output_stm_kp1;
    //////////////////////////////////////////////////////////
    // Connections WITHIN modules:
    // FIFOS within compute:
    tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_0_kp0; tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_1_kp0; tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_2_kp0; tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_3_kp0;
    tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_0_kp1; tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_1_kp1; tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_2_kp1; tapa::streams<ap_uint<(32)>, (7)> key_tmp_stream_3_kp1;
    tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb0_streams_kp0; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb1_streams_kp0; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb2_streams_kp0; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb3_streams_kp0; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb4_streams_kp0; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb5_streams_kp0; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb6_streams_kp0;
    tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb0_streams_kp1; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb1_streams_kp1; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb2_streams_kp1; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb3_streams_kp1; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb4_streams_kp1; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb5_streams_kp1; tapa::streams<PACKED_HASH_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> arb6_streams_kp1;
    tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf0_stm_kp0; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf1_stm_kp0; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf2_stm_kp0; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf3_stm_kp0; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf4_stm_kp0; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf5_stm_kp0; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf6_stm_kp0;
    tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf0_stm_kp1; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf1_stm_kp1; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf2_stm_kp1; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf3_stm_kp1; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf4_stm_kp1; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf5_stm_kp1; tapa::streams<BV_PLUS_IIDX_PACKED_DTYPE, (4)*(8) /* each sub bv is further partitioned into this chunks*/> inter_shuf6_stm_kp1;
    //////////////////////////////////////////////////////////
    // MODULE INVOCATIONS.
    tapa::task()
        .invoke(loadBV
        ...
        .invoke(computeHash_Feeder, 0, 0, key_stream_kp0[0], key_tmp_stream_0_kp0 ) .invoke(computeHash_Feeder, 1, 0, key_stream_kp0[1], key_tmp_stream_1_kp0 ) .invoke(computeHash_Feeder, 2, 0, key_stream_kp0[2], key_tmp_stream_2_kp0 ) .invoke(computeHash_Feeder, 3, 0, key_stream_kp0[3], key_tmp_stream_3_kp0 ) /* Need NUM_STM of these^ invokes */ .invoke(computeHash_Computer, 0, 0, 0, key_tmp_stream_0_kp0[0], hash_stream_h0_kp0[0] ) .invoke(computeHash_Computer, 0, 1, 0, key_tmp_stream_0_kp0[1], hash_stream_h1_kp0[0] ) .invoke(computeHash_Computer, 0, 2, 0, key_tmp_stream_0_kp0[2], hash_stream_h2_kp0[0] ) .invoke(computeHash_Computer, 0, 3, 0, key_tmp_stream_0_kp0[3], hash_stream_h3_kp0[0] ) .invoke(computeHash_Computer, 0, 4, 0, key_tmp_stream_0_kp0[4], hash_stream_h4_kp0[0] ) .invoke(computeHash_Computer, 0, 5, 0, key_tmp_stream_0_kp0[5], hash_stream_h5_kp0[0] ) .invoke(computeHash_Computer, 0, 6, 0, key_tmp_stream_0_kp0[6], hash_stream_h6_kp0[0] ) .invoke(computeHash_Computer, 1, 0, 0, key_tmp_stream_1_kp0[0], hash_stream_h0_kp0[1] ) .invoke(computeHash_Computer, 1, 1, 0, key_tmp_stream_1_kp0[1], hash_stream_h1_kp0[1] ) .invoke(computeHash_Computer, 1, 2, 0, key_tmp_stream_1_kp0[2], hash_stream_h2_kp0[1] ) .invoke(computeHash_Computer, 1, 3, 0, key_tmp_stream_1_kp0[3], hash_stream_h3_kp0[1] ) .invoke(computeHash_Computer, 1, 4, 0, key_tmp_stream_1_kp0[4], hash_stream_h4_kp0[1] ) .invoke(computeHash_Computer, 1, 5, 0, key_tmp_stream_1_kp0[5], hash_stream_h5_kp0[1] ) .invoke(computeHash_Computer, 1, 6, 0, key_tmp_stream_1_kp0[6], hash_stream_h6_kp0[1] ) .invoke(computeHash_Computer, 2, 0, 0, key_tmp_stream_2_kp0[0], hash_stream_h0_kp0[2] ) .invoke(computeHash_Computer, 2, 1, 0, key_tmp_stream_2_kp0[1], hash_stream_h1_kp0[2] ) .invoke(computeHash_Computer, 2, 2, 0, key_tmp_stream_2_kp0[2], hash_stream_h2_kp0[2] ) .invoke(computeHash_Computer, 2, 3, 0, key_tmp_stream_2_kp0[3], hash_stream_h3_kp0[2] ) .invoke(computeHash_Computer, 2, 4, 0, key_tmp_stream_2_kp0[4], hash_stream_h4_kp0[2] ) .invoke(computeHash_Computer, 2, 5, 0, key_tmp_stream_2_kp0[5], hash_stream_h5_kp0[2] ) .invoke(computeHash_Computer, 2, 6, 0, key_tmp_stream_2_kp0[6], hash_stream_h6_kp0[2] ) .invoke(computeHash_Computer, 3, 0, 0, key_tmp_stream_3_kp0[0], hash_stream_h0_kp0[3] ) .invoke(computeHash_Computer, 3, 1, 0, key_tmp_stream_3_kp0[1], hash_stream_h1_kp0[3] ) .invoke(computeHash_Computer, 3, 2, 0, key_tmp_stream_3_kp0[2], hash_stream_h2_kp0[3] ) .invoke(computeHash_Computer, 3, 3, 0, key_tmp_stream_3_kp0[3], hash_stream_h3_kp0[3] ) .invoke(computeHash_Computer, 3, 4, 0, key_tmp_stream_3_kp0[4], hash_stream_h4_kp0[3] ) .invoke(computeHash_Computer, 3, 5, 0, key_tmp_stream_3_kp0[5], hash_stream_h5_kp0[3] ) .invoke(computeHash_Computer, 3, 6, 0, key_tmp_stream_3_kp0[6], hash_stream_h6_kp0[3] ) /* Need NUM_STM of these^ invokes */
        .invoke(computeHash_Feeder, 0, 1, key_stream_kp1[0], key_tmp_stream_0_kp1 ) .invoke(computeHash_Feeder, 1, 1, key_stream_kp1[1], key_tmp_stream_1_kp1 ) .invoke(computeHash_Feeder, 2, 1, key_stream_kp1[2], key_tmp_stream_2_kp1 ) .invoke(computeHash_Feeder, 3, 1, key_stream_kp1[3], key_tmp_stream_3_kp1 ) /* Need NUM_STM of these^ invokes */ .invoke(computeHash_Computer, 0, 0, 1, key_tmp_stream_0_kp1[0], hash_stream_h0_kp1[0] ) .invoke(computeHash_Computer, 0, 1, 1, key_tmp_stream_0_kp1[1], hash_stream_h1_kp1[0] ) .invoke(computeHash_Computer, 0, 2, 1, key_tmp_stream_0_kp1[2], hash_stream_h2_kp1[0] ) .invoke(computeHash_Computer, 0, 3, 1, key_tmp_stream_0_kp1[3], hash_stream_h3_kp1[0] ) .invoke(computeHash_Computer, 0, 4, 1, key_tmp_stream_0_kp1[4], hash_stream_h4_kp1[0] ) .invoke(computeHash_Computer, 0, 5, 1, key_tmp_stream_0_kp1[5], hash_stream_h5_kp1[0] ) .invoke(computeHash_Computer, 0, 6, 1, key_tmp_stream_0_kp1[6], hash_stream_h6_kp1[0] ) .invoke(computeHash_Computer, 1, 0, 1, key_tmp_stream_1_kp1[0], hash_stream_h0_kp1[1] ) .invoke(computeHash_Computer, 1, 1, 1, key_tmp_stream_1_kp1[1], hash_stream_h1_kp1[1] ) .invoke(computeHash_Computer, 1, 2, 1, key_tmp_stream_1_kp1[2], hash_stream_h2_kp1[1] ) .invoke(computeHash_Computer, 1, 3, 1, key_tmp_stream_1_kp1[3], hash_stream_h3_kp1[1] ) .invoke(computeHash_Computer, 1, 4, 1, key_tmp_stream_1_kp1[4], hash_stream_h4_kp1[1] ) .invoke(computeHash_Computer, 1, 5, 1, key_tmp_stream_1_kp1[5], hash_stream_h5_kp1[1] ) .invoke(computeHash_Computer, 1, 6, 1, key_tmp_stream_1_kp1[6], hash_stream_h6_kp1[1] ) .invoke(computeHash_Computer, 2, 0, 1, key_tmp_stream_2_kp1[0], hash_stream_h0_kp1[2] ) .invoke(computeHash_Computer, 2, 1, 1, key_tmp_stream_2_kp1[1], hash_stream_h1_kp1[2] ) .invoke(computeHash_Computer, 2, 2, 1, key_tmp_stream_2_kp1[2], hash_stream_h2_kp1[2] ) .invoke(computeHash_Computer, 2, 3, 1, key_tmp_stream_2_kp1[3], hash_stream_h3_kp1[2] ) .invoke(computeHash_Computer, 2, 4, 1, key_tmp_stream_2_kp1[4], hash_stream_h4_kp1[2] ) .invoke(computeHash_Computer, 2, 5, 1, key_tmp_stream_2_kp1[5], hash_stream_h5_kp1[2] ) .invoke(computeHash_Computer, 2, 6, 1, key_tmp_stream_2_kp1[6], hash_stream_h6_kp1[2] ) .invoke(computeHash_Computer, 3, 0, 1, key_tmp_stream_3_kp1[0], hash_stream_h0_kp1[3] ) .invoke(computeHash_Computer, 3, 1, 1, key_tmp_stream_3_kp1[1], hash_stream_h1_kp1[3] ) .invoke(computeHash_Computer, 3, 2, 1, key_tmp_stream_3_kp1[2], hash_stream_h2_kp1[3] ) .invoke(computeHash_Computer, 3, 3, 1, key_tmp_stream_3_kp1[3], hash_stream_h3_kp1[3] ) .invoke(computeHash_Computer, 3, 4, 1, key_tmp_stream_3_kp1[4], hash_stream_h4_kp1[3] ) .invoke(computeHash_Computer, 3, 5, 1, key_tmp_stream_3_kp1[5], hash_stream_h5_kp1[3] ) .invoke(computeHash_Computer, 3, 6, 1, key_tmp_stream_3_kp1[6], hash_stream_h6_kp1[3] ) /* Need NUM_STM of these^ invokes */
        .invoke( bloom_arb_forwarder, 0, 0, hash_stream_h0_kp0, arb0_streams_kp0 ) .invoke( bloom_arb_forwarder, 1, 0, hash_stream_h1_kp0, arb1_streams_kp0 ) .invoke( bloom_arb_forwarder, 2, 0, hash_stream_h2_kp0, arb2_streams_kp0 ) .invoke( bloom_arb_forwarder, 3, 0, hash_stream_h3_kp0, arb3_streams_kp0 ) .invoke( bloom_arb_forwarder, 4, 0, hash_stream_h4_kp0, arb4_streams_kp0 ) .invoke( bloom_arb_forwarder, 5, 0, hash_stream_h5_kp0, arb5_streams_kp0 ) .invoke( bloom_arb_forwarder, 6, 0, hash_stream_h6_kp0, arb6_streams_kp0 ) .invoke( bloom_single_arbiter, 0, 0, arb0_streams_kp0, bv_lookup_stream_h0_kp0 ) .invoke( bloom_single_arbiter, 1, 0, arb1_streams_kp0, bv_lookup_stream_h1_kp0 ) .invoke( bloom_single_arbiter, 2, 0, arb2_streams_kp0, bv_lookup_stream_h2_kp0 ) .invoke( bloom_single_arbiter, 3, 0, arb3_streams_kp0, bv_lookup_stream_h3_kp0 ) .invoke( bloom_single_arbiter, 4, 0, arb4_streams_kp0, bv_lookup_stream_h4_kp0 ) .invoke( bloom_single_arbiter, 5, 0, arb5_streams_kp0, bv_lookup_stream_h5_kp0 ) .invoke( bloom_single_arbiter, 6, 0, arb6_streams_kp0, bv_lookup_stream_h6_kp0 )
        .invoke( bloom_arb_forwarder, 0, 1, hash_stream_h0_kp1, arb0_streams_kp1 ) .invoke( bloom_arb_forwarder, 1, 1, hash_stream_h1_kp1, arb1_streams_kp1 ) .invoke( bloom_arb_forwarder, 2, 1, hash_stream_h2_kp1, arb2_streams_kp1 ) .invoke( bloom_arb_forwarder, 3, 1, hash_stream_h3_kp1, arb3_streams_kp1 ) .invoke( bloom_arb_forwarder, 4, 1, hash_stream_h4_kp1, arb4_streams_kp1 ) .invoke( bloom_arb_forwarder, 5, 1, hash_stream_h5_kp1, arb5_streams_kp1 ) .invoke( bloom_arb_forwarder, 6, 1, hash_stream_h6_kp1, arb6_streams_kp1 ) .invoke( bloom_single_arbiter, 0, 1, arb0_streams_kp1, bv_lookup_stream_h0_kp1 ) .invoke( bloom_single_arbiter, 1, 1, arb1_streams_kp1, bv_lookup_stream_h1_kp1 ) .invoke( bloom_single_arbiter, 2, 1, arb2_streams_kp1, bv_lookup_stream_h2_kp1 ) .invoke( bloom_single_arbiter, 3, 1, arb3_streams_kp1, bv_lookup_stream_h3_kp1 ) .invoke( bloom_single_arbiter, 4, 1, arb4_streams_kp1, bv_lookup_stream_h4_kp1 ) .invoke( bloom_single_arbiter, 5, 1, arb5_streams_kp1, bv_lookup_stream_h5_kp1 ) .invoke( bloom_single_arbiter, 6, 1, arb6_streams_kp1, bv_lookup_stream_h6_kp1 )
        .invoke( queryResult_per_hash , 0 , bv_load_stream_0 , bv_lookup_stream_h0_kp0 , bv_lookup_stream_h0_kp1 , query_bv_packed_stream_hash0_kp0 , query_bv_packed_stream_hash0_kp1 ) .invoke( queryResult_per_hash , 1 , bv_load_stream_1 , bv_lookup_stream_h1_kp0 , bv_lookup_stream_h1_kp1 , query_bv_packed_stream_hash1_kp0 , query_bv_packed_stream_hash1_kp1 ) .invoke( queryResult_per_hash , 2 , bv_load_stream_2 , bv_lookup_stream_h2_kp0 , bv_lookup_stream_h2_kp1 , query_bv_packed_stream_hash2_kp0 , query_bv_packed_stream_hash2_kp1 ) .invoke( queryResult_per_hash , 3 , bv_load_stream_3 , bv_lookup_stream_h3_kp0 , bv_lookup_stream_h3_kp1 , query_bv_packed_stream_hash3_kp0 , query_bv_packed_stream_hash3_kp1 ) .invoke( queryResult_per_hash , 4 , bv_load_stream_4 , bv_lookup_stream_h4_kp0 , bv_lookup_stream_h4_kp1 , query_bv_packed_stream_hash4_kp0 , query_bv_packed_stream_hash4_kp1 ) .invoke( queryResult_per_hash , 5 , bv_load_stream_5 , bv_lookup_stream_h5_kp0 , bv_lookup_stream_h5_kp1 , query_bv_packed_stream_hash5_kp0 , query_bv_packed_stream_hash5_kp1 ) .invoke( queryResult_per_hash , 6 , bv_load_stream_6 , bv_lookup_stream_h6_kp0 , bv_lookup_stream_h6_kp1 , query_bv_packed_stream_hash6_kp0 , query_bv_packed_stream_hash6_kp1 )
        .invoke<tapa::detach>( shuffle_TtoS_per_hash , 0 , 0 , query_bv_packed_stream_hash0_kp0 , inter_shuf0_stm_kp0 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 1 , 0 , query_bv_packed_stream_hash1_kp0 , inter_shuf1_stm_kp0 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 2 , 0 , query_bv_packed_stream_hash2_kp0 , inter_shuf2_stm_kp0 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 3 , 0 , query_bv_packed_stream_hash3_kp0 , inter_shuf3_stm_kp0 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 4 , 0 , query_bv_packed_stream_hash4_kp0 , inter_shuf4_stm_kp0 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 5 , 0 , query_bv_packed_stream_hash5_kp0 , inter_shuf5_stm_kp0 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 6 , 0 , query_bv_packed_stream_hash6_kp0 , inter_shuf6_stm_kp0 ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,0 ,0 ,inter_shuf0_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,1 ,0 ,inter_shuf0_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,2 ,0 ,inter_shuf0_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,3 ,0 ,inter_shuf0_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,0 ,0 ,inter_shuf1_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,1 ,0 ,inter_shuf1_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,2 ,0 ,inter_shuf1_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,3 ,0 ,inter_shuf1_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,0 ,0 ,inter_shuf2_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,1 ,0 ,inter_shuf2_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,2 ,0 ,inter_shuf2_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,3 ,0 ,inter_shuf2_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,0 ,0 ,inter_shuf3_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,1 ,0 ,inter_shuf3_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,2 ,0 ,inter_shuf3_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,3 ,0 ,inter_shuf3_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,0 ,0 ,inter_shuf4_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,1 ,0 ,inter_shuf4_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,2 ,0 ,inter_shuf4_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,3 ,0 ,inter_shuf4_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,0 ,0 ,inter_shuf5_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,1 ,0 ,inter_shuf5_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,2 ,0 ,inter_shuf5_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,3 ,0 ,inter_shuf5_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,0 ,0 ,inter_shuf6_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp0[6] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,1 ,0 ,inter_shuf6_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp0[6] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,2 ,0 ,inter_shuf6_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp0[6] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,3 ,0 ,inter_shuf6_stm_kp0[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp0[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp0[6] )
        .invoke<tapa::detach>( shuffle_TtoS_per_hash , 0 , 1 , query_bv_packed_stream_hash0_kp1 , inter_shuf0_stm_kp1 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 1 , 1 , query_bv_packed_stream_hash1_kp1 , inter_shuf1_stm_kp1 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 2 , 1 , query_bv_packed_stream_hash2_kp1 , inter_shuf2_stm_kp1 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 3 , 1 , query_bv_packed_stream_hash3_kp1 , inter_shuf3_stm_kp1 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 4 , 1 , query_bv_packed_stream_hash4_kp1 , inter_shuf4_stm_kp1 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 5 , 1 , query_bv_packed_stream_hash5_kp1 , inter_shuf5_stm_kp1 ) .invoke<tapa::detach>( shuffle_TtoS_per_hash , 6 , 1 , query_bv_packed_stream_hash6_kp1 , inter_shuf6_stm_kp1 ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,0 ,1 ,inter_shuf0_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf0_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,1 ,1 ,inter_shuf0_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf0_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,2 ,1 ,inter_shuf0_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf0_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,0 ,3 ,1 ,inter_shuf0_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf0_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[0] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,0 ,1 ,inter_shuf1_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf1_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,1 ,1 ,inter_shuf1_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf1_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,2 ,1 ,inter_shuf1_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf1_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,1 ,3 ,1 ,inter_shuf1_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf1_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[1] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,0 ,1 ,inter_shuf2_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf2_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,1 ,1 ,inter_shuf2_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf2_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,2 ,1 ,inter_shuf2_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf2_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,2 ,3 ,1 ,inter_shuf2_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf2_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[2] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,0 ,1 ,inter_shuf3_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf3_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,1 ,1 ,inter_shuf3_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf3_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,2 ,1 ,inter_shuf3_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf3_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,3 ,3 ,1 ,inter_shuf3_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf3_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[3] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,0 ,1 ,inter_shuf4_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf4_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,1 ,1 ,inter_shuf4_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf4_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,2 ,1 ,inter_shuf4_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf4_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,4 ,3 ,1 ,inter_shuf4_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf4_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[4] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,0 ,1 ,inter_shuf5_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf5_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,1 ,1 ,inter_shuf5_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf5_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,2 ,1 ,inter_shuf5_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf5_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,5 ,3 ,1 ,inter_shuf5_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf5_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[5] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,0 ,1 ,inter_shuf6_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*0] ,inter_shuf6_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*0] ,reconstruct_stream_stm0_kp1[6] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,1 ,1 ,inter_shuf6_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*1] ,inter_shuf6_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*1] ,reconstruct_stream_stm1_kp1[6] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,2 ,1 ,inter_shuf6_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*2] ,inter_shuf6_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*2] ,reconstruct_stream_stm2_kp1[6] ) .invoke<tapa::detach>( shuffle_reordering_per_hash ,6 ,3 ,1 ,inter_shuf6_stm_kp1[0+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[1+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[2+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[3+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[4+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[5+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[6+(8) /* each sub bv is further partitioned into this chunks*/*3] ,inter_shuf6_stm_kp1[7+(8) /* each sub bv is further partitioned into this chunks*/*3] ,reconstruct_stream_stm3_kp1[6] )
        .invoke(bloom_aggregate_SPLIT, 0, 0, reconstruct_stream_stm0_kp0, aggregate_stream_kp0[0] ) .invoke(bloom_aggregate_SPLIT, 1, 0, reconstruct_stream_stm1_kp0, aggregate_stream_kp0[1] ) .invoke(bloom_aggregate_SPLIT, 2, 0, reconstruct_stream_stm2_kp0, aggregate_stream_kp0[2] ) .invoke(bloom_aggregate_SPLIT, 3, 0, reconstruct_stream_stm3_kp0, aggregate_stream_kp0[3] )
        .invoke(bloom_aggregate_SPLIT, 0, 1, reconstruct_stream_stm0_kp1, aggregate_stream_kp1[0] ) .invoke(bloom_aggregate_SPLIT, 1, 1, reconstruct_stream_stm1_kp1, aggregate_stream_kp1[1] ) .invoke(bloom_aggregate_SPLIT, 2, 1, reconstruct_stream_stm2_kp1, aggregate_stream_kp1[2] ) .invoke(bloom_aggregate_SPLIT, 3, 1, reconstruct_stream_stm3_kp1, aggregate_stream_kp1[3] )
