...
int main(int argc, char **argv) {
    ...
    else if (argc == 5) {//for testing alpha, beta
        ALPHA = atof(argv[3]);
        BETA = atof(argv[4]);
    }
    else if (argc == 4) {//for testing rp_time
        rp_time = atoi(argv[3]);
    }
    else if (argc != 3) {//argc == 3 for testing N
        cout << "Usage: " << argv[0] << " [matrix A file] [N] [rp_time] [alpha] [beta]" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    for (int nn = 0; nn < N; ++nn) {
        for (int kk = 0; kk < K; ++kk) {
            mat_B_cpu[kk + K * nn] = 1.0;//(1.0 + kk) + 0.1 * (1.0 + nn); //100.0 * (kk + 1)  + 1.0 * (nn + 1);// / K / N;
        }
    }
    ...
    for (int nn = 0; nn < N; ++nn) {
        for (int mm = 0; mm < M; ++mm) {
            ...
            //cout << "n = " << nn << ", m = " << mm << ", cpu = " << v_cpu << ", fpga = " << v_fpga << endl;
        }
    }
