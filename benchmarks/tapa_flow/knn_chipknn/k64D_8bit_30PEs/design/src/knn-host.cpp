...
DATA_TYPE square_and_handle_overflow(DATA_TYPE input)
{
    ...
    else if ( (fabs(input.to_float()) >= FLOOR_SQRT_MAX_DATA_TYPE_VAL) ) {
        return MAX_DATA_TYPE_VAL;
    }
    else {
        return input * input;
                                ...
                                DATA_TYPE data_point)
{
    ...
    if (DISTANCE_METRIC == 0){
        delta = fabs( add_and_handle_overflow(query_point,  -1*data_point).to_float() );
...
bool verify(std::vector<DATA_TYPE, tapa::aligned_allocator<DATA_TYPE>> &sw_dist,
            std::vector<INT32, tapa::aligned_allocator<INT32>> &hw_dist_raw,
            ...
            unsigned int size)
{
    ...
    // Workaround for a problem with non-32-bit-aligned data widths.
    for (int i = 0; i < size; ++i)
    {
        hw_dist[i].range(DATA_TYPE_TOTAL_SZ-1, 0) = hw_dist_raw[i].range(DATA_TYPE_TOTAL_SZ-1, 0);
    }
    ...
    for (unsigned int i=0; i<size; ++i) {
        ...
        std::cout << " CPU result = " << sw_id[i] << " : " << sw_dist[i]
                  ...
                  << std::endl
                  << " Delta = " << fabs(sw_dist[i].to_float() - hw_dist[i].to_float())
                  << std::endl;
                            ...
                            unsigned int num_of_points)
{
    ...
    float max_generated_val = (FLOOR_SQRT_MAX_DATA_TYPE_VAL > MAX_DATA_TYPE_VAL) ? MAX_DATA_TYPE_VAL : FLOOR_SQRT_MAX_DATA_TYPE_VAL;
    max_generated_val = max_generated_val * 2 / INPUT_DIM;

    for (unsigned int i=0; i<INPUT_DIM; ++i){
        query[i] = max_generated_val/2;
        printf("Query[%d] = %f\n", i, query[i].to_float());
    }
    ...
    for (unsigned long long int i=0; i<num_of_points*INPUT_DIM; ++i){
        #if DATA_TYPE_TOTAL_SZ > 20
        // For more bits in the fixed point payload
            float cur_val = static_cast<float>(
                                static_cast<float>(rand()) / static_cast<float>(RAND_MAX)
                             )*static_cast<float>(max_generated_val);
            searchSpace[i] = static_cast<DATA_TYPE>(cur_val);

        #else
        // If we have a small-bitwidth fixed point payload, we need to constrain the inputs
        //   because there will be collisions.
        if ( i/INPUT_DIM >= start_of_non_max_values && i/INPUT_DIM < start_of_non_max_values+num_of_non_max_values){
            ...
            searchSpace[i] = MAX_DATA_TYPE_VAL;
        }
        #endif
    }
...
int main(int argc, char* argv[]) {
    ...
    printf("KDEBUG: NUM_BITS_PER_KRNL_PADDED = %d\n\n", NUM_BITS_PER_KRNL_PADDED);
    ...
    // Workaround for a problem with non-32-bit-aligned data widths.
    vector<INT32, tapa::aligned_allocator<INT32>> hw_dist_raw(TOP);
    ...
    std::fill(hw_dist_raw.begin(), hw_dist_raw.end(), 0.0);
    ...
    int data_packing_factor = (IWIDTH/DATA_TYPE_TOTAL_SZ);
    ...
    for (int i(0); i<NUM_PE; ++i) {
        ...
        for (int j(0); j<formatted_length; ++j) {
            ...
            for (int k(0); k<data_packing_factor; ++k) {
                ...
                tmp.range(k*DATA_TYPE_TOTAL_SZ+ (DATA_TYPE_TOTAL_SZ - 1),
                          k*DATA_TYPE_TOTAL_SZ) = this_ele.range(DATA_TYPE_TOTAL_SZ - 1, 0);
            }
            in[i][j] = tmp;
        }
    }
    ...
    int64_t kernel_time_ns = tapa::invoke(Knn, FLAGS_bitstream,
                 ...
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[21]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[22]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[23]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[24]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[25]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[26]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[27]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[28]),
                 tapa::read_only_mmap<INTERFACE_WIDTH>(in[29]),
                 tapa::write_only_mmap<INT32>(hw_dist_raw),
                 tapa::write_only_mmap<int>(hw_id)
    );
    ...
    // verify results
    bool match = true;
    match = verify(sw_dist, hw_dist_raw, hw_dist, sw_id, hw_id, query_data, searchspace_data, TOP);
